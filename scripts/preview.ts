...
async function main() {
  ...
  if (!app) {
    console.log('Usage: pnpm preview <app> [-- ...args]');
    console.log('       pnpm preview --app <app> [-- ...args]');
  ...
  const child = spawn('pnpm', ['-C', appPath, 'run', 'preview', ...(rest.length ? ['--', ...rest] : [])], { stdio: 'inherit', shell: true });
