...
function parseArgs(argv: string[]): Args {
  ...
  while (i < argv.length) {
    const a = argv[i];
    if (a === '--app' || a === '-a') { out.app = argv[i + 1]; i += 2; continue; }
    if (!a.startsWith('-') && !out.app) { out.app = a; i += 1; continue; }
    out.rest.push(a); i += 1;
  }
  return out;
}
...
async function listApps(): Promise<string[]> {
  try {
    const es = await fsp.readdir(appsDir, {withFileTypes: true});
...
async function main() {
  ...
  if (!app) {
    console.log('Usage: pnpm build:app <app> [-- ...args]');
    console.log('       pnpm build:app --app <app> [-- ...args]');
  ...
  const child = spawn('pnpm', ['-C', appPath, 'run', 'build', ...(rest.length ? ['--', ...rest] : [])], { stdio: 'inherit', shell: true });
